...
open class Curve: Parametric {
    ...
    public static let linear                = Curve(parametricFunction: {t in return t}) as Parametric
...
open class CompositeCurve: Parametric {
	...
	private class func internalInit(format: String, args: [Any]) throws -> (startValue: Double, segments: [Segment]) {
        ...
        guard format.utf8CString.count == args.count else {
		...
		var idx: Int = 0
		for char in format.utf8CString {
			switch char {
            case is Int8: // StartValue
				guard idx == 0 else {
